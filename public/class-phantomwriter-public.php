 ...
 * @subpackage Phantomwriter/public
 */
...
class Phantomwriter_Public
{
	...
	public function phantomwriter_client_enqueue_styles()
	{
		...
		wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/phantomwriter-public.css', array(), $this->version, 'all');
	...
	public function phantomwriter_client_enqueue_scripts()
	{
		...
		wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/phantomwriter-public.js', array('jquery'), $this->version, false);
	}

	public function redirect_on_activate()
	{
		if (get_option('phantomwriter_client_do_activation_redirect', false)) {
			delete_option('phantomwriter_client_do_activation_redirect');
			if (!isset($_GET['activate-multi'])) {
				wp_safe_redirect(PHANTOMWRITER_CLIENT_SETTINGS_URL);
				exit;
			}
		}
	}
